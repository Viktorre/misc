...
##### define rolling time window last training years
####
#

last_training_years = [2007]#,2010,2011,2012,2013,2014]
#last_training_years = [2009,2010,2011,2012,2013,2014]

#
###
##### def last training years
...
######    tune hyperparams (alpha, l1_ratio)
### 
#
...
def get_avg_oos_r2_from_one_predictor(df,predictor,grid,last_training_years=last_training_years):

    predictor_avg_r2 = []

    for last_training_year in last_training_years: #rolling time window
...
PLSRegression(n_components=50), #needs tuning # -2.28
custom_pcr(LinearRegression(),50), # K=1000 for pcr and pls in my analysis
...
for one_predictor in predictors:
    ...
    if one_predictor.__class__.__name__ != 'VotingRegressor':
        ...
        r2_results[one_predictor.__class__.__name__] = [get_avg_oos_r2_from_one_predictor(df,one_predictor,grid_dict)]

    else:
        
        print(str('NN'+str(len(one_predictor.estimators[0][1].hidden_layer_sizes))))
    
        r2_results[str('NN'+str(len(one_predictor.estimators[0][1].hidden_layer_sizes)))] = [get_avg_oos_r2_from_one_predictor(df,one_predictor,grid_dict)]

    print('\n')

print(r2_results)

r2_results.to_csv(path+'r2_result_table.csv')
...
def return_dm_table(array_of_predictors,df,last_training_years=last_training_years):
        ...
        for second_predictor in array_of_predictors:
            ...
            if array_of_predictors.index(first_predictor) < array_of_predictors.index(second_predictor):#condition to get all unique pairs of predictors
                ...
                if False:#grid[predictor.__class__.__name__]: #tunable estimators use grid, the rest only plain predictor # i do this condition by giving non tunable estimators FALSE in their grid
                    ...
                    for last_training_year in last_training_years:#[2009,2010,2011,2012,2013,2014]: #rolling time window
...
#                    result_array.append(np.array(dm.dm_test(all_y_true[0].to_list(), all_pred1[0].to_list(), all_pred2[0].to_list(),h=1, crit="MSE")))#,
                    result_tuple = np.array(dm.dm_test(all_y_true[0].to_list(), all_pred1[0].to_list(), all_pred2[0].to_list(),h=1, crit="MSE"))
                    if result_tuple[1] < 0.05: #cond to filter significant test stats
                        result_array.append(str(round(result_tuple[0], 2))+'*')
                    else:
                        result_array.append(str(round(result_tuple[0], 2))+' ')       
            else:
...
custom_ols3(HuberRegressor()),
PLSRegression(n_components=50), #needs tuning # -2.28
custom_pcr(LinearRegression(),50),
#ElasticNet(random_state=0,alpha=0.005,l1_ratio=0.5), # 0.099 #alpha tunable
#RandomForestRegressor(max_depth=3,n_estimators=300,min_samples_split=20), #min_samples_split and max_depth tunable
#GradientBoostingRegressor(learning_rate=0.05,n_estimators=500,max_depth=2), 
ensemble_of_NN1,
...
#dm_table.to_csv(path+'r2_result_table.csv')
